...
side  set  values  should  be  applied  to  the  PINDIRs  and  not  the  PINs.  This  directive  is  only  valid  within  a  program before the first instruction.
...
irq <irq_num> (prev, rel, next) irq set <irq_num> (prev, rel, next) irq nowait <irq_num> (prev, rel, next) irq wait <irq_num> (prev, rel, next) irq clear <irq_num> (prev, rel, next) where:
...
<codeblock>|||| 8 #include "auto_push_pull.pio.h"
...
||||----------
Figure 49. Execution of auto_push_pull program. The state machine stalls on an OUT until data has travelled through the TX FIFO into the OSR.
...
bypasses the ISR and To trigger automatic push or pull at the correct time, the state machine tracks the total shift count of the ISR and OSR, goes straight to the RX FIFO. The state machine stalls again when the FIFO has drained, and the OSR is once again empty.
...
<codeblock>|||| 1 isr = shift_in(isr, input())
|||| 1 isr = shift_in(isr, input())
|||| 2 isr count = saturate(isr count + in count)
 3

<codeblock>|||| 4 if rx count >= threshold:
...
<codeblock>||||1 if MOV or PULL:
||||1 if MOV or PULL:
</codeblock>

</codeblock>

<codeblock>||||2     osr count = 0
||||2     osr count = 0
</codeblock>
...
<codeblock>||||4 if osr count >= threshold:
||||4 if osr count >= threshold:
</codeblock>

</codeblock>

<codeblock>||||5     if tx fifo not empty:
||||5     if tx fifo not empty:
</codeblock>

</codeblock>

<codeblock>||||6         osr = pull()
||||6         osr = pull()
</codeblock>

</codeblock>

<codeblock>||||7         osr count = 0
||||7         osr count = 0
</codeblock>
...
<codeblock>|||| 1 if osr count >= threshold:
